...
def weights_init(m, gain=0.02):
    ...
    for _, cell in m.cells_and_names():
        if isinstance(cell, nn.Conv2d):
            cell.weight.set_data(init.initializer(init=init.Normal(sigma=gain, mean=0.0), shape=cell.weight.shape))
        if isinstance(cell, nn.BatchNorm2d):
            cell.gamma.set_data(init.initializer(init=init.Normal(sigma=gain, mean=1.0), shape=cell.gamma.shape))
