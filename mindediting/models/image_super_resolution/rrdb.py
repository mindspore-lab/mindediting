...
from mindediting.models.common.resblock_with_input_conv import make_layer
    ...
    def construct(self, x):
        ...
        if self.scale >= 2:
            feat = ms.ops.interpolate(
                feat, scales=(1.0, 1.0, 2.0, 2.0), coordinate_transformation_mode="half_pixel", mode="bilinear"
            )
        ...
        out = self.conv_last(self.lrelu(feat_hr))
        out = out.clip(0, 1)
        if self.scale_to_0_1:
            out = out * 2 - 1
        return out
