    ...
    def __init__(
        ...
        for _, m in self.cells_and_names():
            if isinstance(m, nn.Conv2d):
                ...
                m.weight = initializer(kaiming_init, m.weight.shape, m.weight.dtype)
            elif isinstance(m, (nn.BatchNorm2d, nn.GroupNorm)):
                m.gamma = initializer(Normal(1.0), m.gamma.shape)
                m.beta = initializer(Zero(), m.beta.shape)
        ...
        if zero_init_residual:
            for _, m in self.cells_and_names():
                if isinstance(m, Bottleneck):
                    m.bn2.weight = fill(ms.float32, m.bn3.weight, 0)  # type: ignore[arg-type]
                elif isinstance(m, BasicBlock):
                    m.bn2.weight = fill(ms.float32, m.bn2.weight, 0)  # type: ignore[arg-type]
