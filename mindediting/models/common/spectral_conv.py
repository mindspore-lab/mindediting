...
class Conv2dNormalized(nn.Cell):
    ...
    def __init__(
        ...
        self.conv2d = ops.Conv2D(
            out_channel=out_channel, kernel_size=kernel_size, stride=stride, mode=1, pad=pad, pad_mode=pad_mode
        )
        ...
        self.weight_orig = Parameter(
            initializer(Normal(sigma=0.02), (out_channel, in_channel, kernel_size, kernel_size)), name="weight_orig"
        )

        self.weight_u = Parameter(self.initialize_param(out_channel, 1), requires_grad=False, name="weight_u")
        self.weight_v = Parameter(
            self.initialize_param(in_channel * kernel_size * kernel_size, 1), requires_grad=False, name="weight_v"
        )
