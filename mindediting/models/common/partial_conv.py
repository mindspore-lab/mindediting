...
class PartialConv2d(nn.Cell):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=0, bias=False, multi_channel=True):
        super().__init__()
        ...
        self.mask_update_conv_ops = ops.Conv2D(
            out_channel=out_channels, kernel_size=kernel_size, pad_mode="pad", pad=padding, stride=stride
        )
        if self.multi_channel:
            self.weight_mask_updater = ops.Ones()((out_channels, in_channels, kernel_size, kernel_size), mstype.float32)
        else:
            self.weight_mask_updater = ops.Ones()((1, 1, kernel_size, kernel_size), mstype.float32)

        self.slide_winsize = (
            self.weight_mask_updater.shape[1] * self.weight_mask_updater.shape[2] * self.weight_mask_updater.shape[3]
        )
...
class PConvBNActiv(nn.Cell):
    ...
    def __init__(self, in_channels, out_channels, bn=True, sample="none-3", activation="relu", bias=False):
        super().__init__()

        if sample == "down-7":
            self.conv = PartialConv2d(
                in_channels, out_channels, kernel_size=7, stride=2, padding=3, bias=bias, multi_channel=True
            )
        elif sample == "down-5":
            self.conv = PartialConv2d(
                in_channels, out_channels, kernel_size=5, stride=2, padding=2, bias=bias, multi_channel=True
            )
        elif sample == "down-3":
            self.conv = PartialConv2d(
                in_channels, out_channels, kernel_size=3, stride=2, padding=1, bias=bias, multi_channel=True
            )
        else:
            self.conv = PartialConv2d(
                in_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=bias, multi_channel=True
            )
        self.bn = None
        if bn:
