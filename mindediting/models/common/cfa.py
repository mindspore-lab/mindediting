...
from mindediting.utils.utils import extract_patches, extract_patches_ascend_opt, is_ascend
...
class RAL(nn.Cell):
    ...
    def __init__(self, kernel_size=3, stride=1, rate=2, softmax_scale=10.0):
        ...
        if is_ascend():
            self.extract_patches = extract_patches_ascend_opt
        else:
    ...
    def construct(self, background, foreground):
        ...
        background_patches = self.extract_patches(
            background, ksize=background_kernel_size, stride=self.stride * self.rate
        )
        background_patches = background_patches.view(
            background_size[0], -1, background_size[1], background_kernel_size, background_kernel_size
        )
        ...
        foreground_patches = self.extract_patches(foreground, ksize=self.kernel_size, stride=self.stride)
        ...
        for i in range(bs):
            ...
            foreground_patches_item_normed = foreground_patches_item / ops.Maximum()(
                ops.Sqrt()(ops.ReduceSum(keep_dims=True)(foreground_patches_item * foreground_patches_item, [1, 2, 3])),
                self.escape_nan,
            )

            out_ch, _, ks, _ = foreground_patches_item_normed.shape
            score_map = ops.Conv2D(out_ch, ks, pad_mode="pad", pad=padding, stride=1)(
                foreground_item, foreground_patches_item_normed
            )
            ...
            attention_map = ops.maximum(attention_map, 1e-8)

            background_patches_item = background_patches_item[0]
            out_ch, _, ks, _ = background_patches_item.shape
            output_shape = self._get_size_conv_transpose2d(
                attention_map, background_patches_item, stride=self.rate, padding=1, dilation=1
            )
            output_item = (
                ops.Conv2DTranspose(out_ch, ks, pad_mode="pad", pad=1, stride=self.rate)(
                    attention_map, background_patches_item, output_shape
                )
                / 4.0
            )
...
class MSFA(nn.Cell):
    ...
    def __init__(self, in_channels=64, out_channels=64, dilation_rate_list=(1, 2, 4, 8)):
        ...
        self.weight_calc = nn.SequentialCell(
            nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=1, pad_mode="pad", padding=1, has_bias=True),
            nn.ReLU(),
