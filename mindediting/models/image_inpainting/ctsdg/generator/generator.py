...
from mindediting.models.common.bigff import BiGFF
from mindediting.models.common.cfa import CFA
from mindediting.models.common.partial_conv import PConvBNActiv
from mindediting.models.common.projection import Feature2Structure, Feature2Texture
from mindediting.utils.initializer import weights_init
...
class Generator(nn.Cell):
    ...
    def __init__(self, image_in_channels=3, edge_in_channels=2, out_channels=3):
        ...
        # Bidirectional Gated Feature Fusion
        ...
        self.out_layer = nn.SequentialCell(nn.Conv2d(64 + 64 + 64, 3, kernel_size=1, has_bias=True), nn.Tanh())
        weights_init(self)
    ...
    def _encoder_stage(inp, mask, encoder):
        ...
        for i in range(7):
            ec_out, ec_out_mask = encoder[i](ec_outputs[-1], ec_outputs_masks[-1])
    ...
    def construct(self, input_image, input_edge, mask):
        ...
        ec_textures, ec_textures_masks = self._encoder_stage(input_image, input_texture_mask, self.ec_texture_layers)

        input_structure_mask = ops.Concat(axis=1)((mask, mask))
        ec_structures, ec_structures_masks = self._encoder_stage(
            input_edge, input_structure_mask, self.ec_structure_layers
        )

        dc_textures = self._decoder_stage(
            ec_structures[-1], ec_structures_masks[-1], ec_textures, ec_textures_masks, self.dc_texture_layers
        )
        dc_structures = self._decoder_stage(
            ec_textures[-1], ec_textures_masks[-1], ec_structures, ec_structures_masks, self.dc_structure_layers
        )
