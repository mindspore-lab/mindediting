...
def create_scheduler(
    ...
    Args:
        ...
        warmup_factor: the warmup phase of scheduler is a linearly increasing lr,
            the beginning factor is `warmup_factor`, i.e.,
            the lr of the first step/epoch is warmup_base_lr*warmup_factor,
            and the ending lr in the warmup phase is lr. Default: 0.0
        warmup_base_lr: warmup learning rate value. Default: equal to lr
        ...
        t_mult (float): Increase T_max by a factor of t_mult (for cosine_annealing_warm_restarts_lr)
    ...
    elif scheduler in ["cosine_annealing", "warmup_cosine_annealing"]:
        main_lr_scheduler = cosine_annealing_lr(
            t_max=main_epochs, eta_min=min_lr, eta_max=lr, steps_per_epoch=steps_per_epoch, epochs=main_epochs
        )
    elif scheduler == "cosine_annealing_warm_restarts_lr":
        main_lr_scheduler = cosine_annealing_warm_restarts_lr(
            te=main_epochs, tm=t_mult, eta_min=min_lr, eta_max=lr, steps_per_epoch=steps_per_epoch, epochs=main_epochs
        )
    elif scheduler == "epoch_decay_lr":
        main_lr_scheduler = epoch_decay_lr(lr=lr, epoch_num=main_epochs, per_device_batch_num=steps_per_epoch)
    else:
