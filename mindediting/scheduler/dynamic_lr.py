...
def epoch_decay_lr(lr, epoch_num, per_device_batch_num):
    lr_list = []
    for n_epoch in range(epoch_num):
        for _ in range(per_device_batch_num):
            lr_list.append(lr)
        if (n_epoch + 1) % 500 == 0:
            lr /= 2
    return lr_list
...
if __name__ == "__main__":
    ...
    table = (
        (
            (
                "constant_lr",
                constant_lr(0.5, 4, lr=0.05, steps_per_epoch=2, epochs=10),
            ),
        ),
        (
