    ...
    **kwargs,
    ...
    Args:
        ...
        opt: optimizer name like 'sgd', 'nesterov', 'momentum', 'adam', 'adamw', 'rmsprop', 'adagrad', 'lamb'.
            Adam is the default choose for convolution-based networks.
    ...
    # adaptive
        ...
        optimizer = AdamW(params=params, learning_rate=lr, weight_decay=weight_decay, loss_scale=loss_scale, **opt_args)
    elif opt == "nadam":
        optimizer = NAdam(
            params=params,
            learning_rate=lr,
        ...
        optimizer = Adan(params=params, learning_rate=lr, weight_decay=weight_decay, loss_scale=loss_scale, **opt_args)
    elif opt == "rmsprop":
        ...
        optimizer = nn.Adagrad(
            params=params, learning_rate=lr, weight_decay=weight_decay, loss_scale=loss_scale, **opt_args
        )
    elif opt == "lamb":
        assert loss_scale == 1.0, "Loss scaler is not supported by Lamb optimizer"
        optimizer = nn.Lamb(params=params, learning_rate=lr, weight_decay=weight_decay, **opt_args)
    elif opt == "gan_adam":
        params_g = params.get("params_generator")
        params_d = params.get("params_discriminator")
        optimizer_generator = nn.Adam(params=params_g.get("params"), learning_rate=params_g.get("lr"), **opt_args)
        optimizer_discriminator = nn.Adam(params=params_d.get("params"), learning_rate=params_d.get("lr"), **opt_args)
        optimizer = {"generator": optimizer_generator, "discriminator": optimizer_discriminator}
